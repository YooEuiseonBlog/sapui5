...
/*
    구성 요소 초기화 방법에서 이제 라우터를 초기화하는 호출을 추가합니다. 
    라우터를 수동으로 인스턴스화할 필요가 없으며 AppDescriptor 구성에 따라 자동으로 인스턴스화되고 구성 요소에 할당됩니다.

    라우터를 초기화하면 현재 URL을 평가하고 해당 보기view를 자동으로 로드합니다. 
    이는 AppDescriptor에 구성된 경로(routes) 및 대상(targets)의 도움으로 수행됩니다. 
    경로(routes)에 도달하면 해당 대상(targets)의 보기(view)가 로드되고 표시됩니다.
*/