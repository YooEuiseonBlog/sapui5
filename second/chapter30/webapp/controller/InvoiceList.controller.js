...
/*
    인보이스 목록의 컨트롤러에 이벤트 핸들러 기능을 추가합니다. 
    이제 송장 목록에서 항목을 클릭하여 세부 정보 페이지로 이동할 차례입니다. 
    도우미(helper) 메서드 getOwnerComponent().getRouter()를 호출하여 앱의 라우터 인스턴스에 액세스합니다. 
    라우터에서 navTo 메서드를 호출하여 라우팅 구성에서 지정한 세부 경로로 이동합니다.

    송장 목록에서 항목을 클릭하면 세부 정보 페이지Detail.view가 표시됩니다.
